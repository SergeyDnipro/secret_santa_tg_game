...
from tools import serialize_game_list
...
def start(message):
    ...
    admin_role = message.chat.id in ADMIN_IDS
    bot.send_message(
        message.chat.id,
        "Welcome to SecretSanta game",
        reply_markup=keyboards.get_main_interface_keyboard(admin_role=admin_role)
    )
...
def handle_message(message):
    if message.text == buttons.NEW_GAME_BUTTON:
        ...
        bot.send_message(message.chat.id, f"Enter game ID and your full name for join:")
        bot.register_next_step_handler(message, join_game)
...
def join_game(message):
    data_list = message.text.split(maxsplit=1)
    game_name = data_list[0]
...
def lock_game(message):
    ...
    bot.send_message(
        message.chat.id,
        result,
        reply_markup=keyboards.get_main_interface_keyboard(admin_role=message.chat.id in ADMIN_IDS)
    )
    bot.register_next_step_handler(message, handle_message)
